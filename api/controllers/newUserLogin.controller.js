...
export const newUserRegister = async (req, res) => {
    ...
    {
        res.status(500).json({message: "New user registration failed...API/developer issue." });
    }
};
...
export const returningUserLogin = async (req, res) => {
    try {
        ...
        const returningUserEmailProvided = await newUserLoginModel.findOne({ email });
        if (!returningUserEmailProvided) {
            return res.status(401).json({message: "User was not found" });
        }

        // ensure password correct:
        const correctPassword = await returningUserEmailProvided.comparePassword(password);
        if (!correctPassword) {
            return res.status(401).json({message: "Unable to find user with these credentials" });
        }

        // create a JWT login session token for the user if email/password matched
        const loginSessionToken = jwt.sign({ id: returningUserEmailProvided._id, email: returningUserEmailProvided.email }, JWT_SECRET, {
            expiresIn: "1d",
        });
    ...
    {
        res.status(500).json({message: "Returning user login failed!...API/developer issue." });
    }
};